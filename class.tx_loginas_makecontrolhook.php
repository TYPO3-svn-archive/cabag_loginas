...
/**
 * Adds 'Login As' Icon to local recordlist action icons
 *
 * $Id: $
 *
 ...
 * @subpackage	loginas
 */
class tx_loginas_makecontrolhook implements localRecordList_actionsHook {
	var $loginAsObj = null;

	public function tx_loginas_makecontrolhook() {
		$this->loginAsObj = new tx_loginas;
	}
	...
	public function makeControl($table, $row, $cells, &$parentObject) {
		if($table == 'fe_users') {
			...
			foreach($cells as $key => $value) {
				if(strpos($value, 'empty') === FALSE && strpos($value, 'clear') === FALSE) {
					$tempcells[$key] = $value;
				}
			}
			$cells = $tempcells;
